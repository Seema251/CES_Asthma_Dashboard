...
# Define data paths
data_files <- list(
  "CES_Datasets.RData" = file.path(getwd(), "data", "CES_Datasets.RData"),
  "ces_4.rds" = file.path(getwd(), "data", "ces_4.rds"),
  "ces_demo.rds" = file.path(getwd(), "data", "ces_demo.rds")
)

# Load datasets 
if (file.exists(data_files[["CES_Datasets.RData"]])) {
  load(data_files[["CES_Datasets.RData"]])
  print("CES_Datasets.RData loaded successfully!")
} else {
  print(paste("Error: Dataset not found at", data_files[["CES_Datasets.RData"]]))
}

if (file.exists(data_files[["ces_4.rds"]])) {
  ces_4 <- readRDS(data_files[["ces_4.rds"]])
  print("ces_4.rds loaded successfully!")
} else {
  print(paste("Error: Dataset not found at", data_files[["ces_4.rds"]]))
}

if (file.exists(data_files[["ces_demo.rds"]])) {
  ces_demo <- readRDS(data_files[["ces_demo.rds"]])
  print("ces_demo.rds loaded successfully!")
} else {
  print(paste("Error: Dataset not found at", data_files[["ces_demo.rds"]]))
}
  ...
  dashboardBody(
    ...
    fluidRow(
      box(title = "Scientific Report", width = 12, solidHeader = TRUE, status = "primary",
          h4("Environment, Social Determinants of Health, and Asthma in California in 2021"),
